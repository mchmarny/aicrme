...
func snapshotNamespace(ctx context.Context, h HelmLister, k kubernetes.Interface, ns string) ([]string, engine.NamespaceRef) {
	...
	obj, err := k.CoreV1().Namespaces().Get(ctx, ns, metav1.GetOptions{})
	switch {
	case apierrors.IsNotFound(err):
		// Not an error: it is the observation that makes this namespace
		// deletable by a later Reset, and the common case for a fresh
		// install.
		return releases, ref
	case err != nil:
