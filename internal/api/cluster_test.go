...
// loginJar authenticates and returns the session cookie every request below
// carries. The connect gate runs after auth, so a test of the gate has to get
// past auth first.
func loginJar(t *testing.T, h http.Handler) []*http.Cookie {
	t.Helper()
	req := httptest.NewRequest(http.MethodPost, "/api/login", strings.NewReader(`{"username":"admin","password":"correct-horse"}`))
